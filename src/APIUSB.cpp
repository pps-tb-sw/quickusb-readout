...
namespace API
{
  ...
  SetControlReg(void)
  {
    ...
    ret[0] = header;
    for (int index=1; index < nwords; index++) { ret[index]=0; }  
  ...
  void
  ReadGetControlReg(unsigned char* input)
  {
  ...
  SetSetupReg(void)
  {
    ...
    ////////////////////////////////// RET 1 ////////////
    ret[1] |= (setup_parity << 6);
    ret[1] |= (enable_ttl_hit << 5);
    ret[1] |= (enable_ttl_clock << 4);
    ret[1] |= (enable_ttl_reset << 3);
    ret[1] |= (enable_ttl_control << 2);
    ret[1] |= (enable_ttl_serial << 1); 
    ret[1] |= (enable_pair << 0); 
    
    // outFile << "ret 1 "<< hex << std::bitset<8>(ret[1]) << endl; // ret 1 OK &  
    
    ////////////////////////////////// RET 2 ////////////
    ret[2] |= (enable_matching << 7);
    ret[2] |= ((roll_over & 4064 ) >> 5);
    
    ////////////////////////////////// RET 3 ////////////
    ret[3] |= ((roll_over & 31) << 3);
    ret[3] |= 3;
    ...
    ////////////////////////////////// RET 6 ////////////
    ret[6] |= ((pll_control & 192) >> 6);
   
    ////////////////////////////////// RET 7////////////
    ret[7] |= ((pll_control & 63) << 2);
    ret[7] |= dll_mode;
    
    ////////////////////////////////// RET 8////////////
    ret[8] = (mode_rc << 7);
    ret[8] |= (mode_rc_compression << 6);
    ret[8] |= (enable_leading << 5);
    ret[8] |= (enable_trailing << 4);
    ...
    ///////////////////////////////////RET 63 ////////////
    ret[63] = (ret[63] | 64);
    ret[63] |= ((trigger_count_offset & 4032) >> 6);
    ...
    ret[74] |= ((reject_count_offset & 3840) >> 8);
  ...
  GetSetupReg(void)
  {
    ...
    const std::string headerstr = readheader ("GetSetupReg");
    unsigned char header = bintohex (headerstr.c_str ());
  ...
  void
  ReadGetSetupReg(unsigned char* input)
  {
    const int nwords = 81;
    unsigned char* ret = new unsigned char[nwords]();
    memcpy(ret, input, nwords);
  ...
  void
  ReadGetStatReg(unsigned char *input)
  {	
    const int nwords = 8;
    unsigned char *ret = new unsigned char [nwords]();
  ...
  unsigned int
  ReadGetIDCode(unsigned char* input)
  {
    const int nwords = 4;
    
    for (unsigned short i=0; i<nwords; i++) {
      std::cout << __PRETTY_FUNCTION__ << " input[" << std::dec << i << "] = " << std::hex << (unsigned short)input[i] << std::endl;
    }

    unsigned long id_code = (((input[3] & 0xff) << 24) | ((input[2] & 0xff) << 16) | ((input[1] & 0xff) << 8)| ((input[0] & 0xff) )) & 0xffffffff;
    std::cout << std::dec << "id_code "<< id_code << std::endl; // outFile in the screen
    
    ofstream outFile("ReadGetIDCode.txt");
    outFile << std::dec << "id_code " << id_code << endl; // put on file
    outFile.close();
  ...
  void
  ReadGetICR(unsigned char* input)
  {
    const int nwords = 94;
    unsigned char *ret = new unsigned char [nwords]();
  ...
  {
    unsigned char res = 0;
    while (*digits) res = (res << 1) | (*digits++ - '0');
    return res;
  }
  ...
  GetSixteenBits(int parameter, const int nwords)
  {
    ...
    for (unsigned int i=0; i<wordsafterheader; i++) {
